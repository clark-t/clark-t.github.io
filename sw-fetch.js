...
self.addEventListener('fetch', function (e) {
    (0, _log.log)('fetch sw: onfetch');

    (0, _helper.grade)('fetchEvent', 1);
    (0, _helper.checkProperties)(e, ['request', 'respondWith'], 1, { prefix: 'fetchEvent.' });
    (0, _log.log)('fetch sw: get url', e.request.url);

    var url = new URL(e.request.url);

    (0, _log.log)('fetch sw: create new url', url.pathname);

    if (/\/whoareyou\.json$/.test(url.pathname)) {
        var mock = { im: 'fetch-sw' };
        (0, _log.log)('fetch sw: mock respone', mock);

        e.respondWith(new Response((0, _stringify2.default)(mock), {
            headers: {
